    ...
    (set, get) => ({
      ...
      fetchGroups: async (orgId) => {
        ...
        try {
          const { data } = await axiosInstance.get(
            `/organizations/user-group/by-org/${orgId}/`,
          );
          // console.log("d", data)
      ...
      createGroup: async (orgId, name, userIds = []) => {
        try {
          ...
          const { data: newGroup } = await axiosInstance.post(
            `/organizations/user-group/`,
            payload,
          );
      ...
      updateGroup: async (groupId, name) => {
        try {
          const { data: updatedGroup } = await axiosInstance.patch(
            `/organizations/user-group/${groupId}/`,
            { name },
          );
          set({
      ...
      addUserToGroup: async (groupId, userId) => {
        try {
          const { data: updatedGroup } = await axiosInstance.post(
            `/organizations/user-group/${groupId}/add_user/`,
            {
              user_id: userId,
            },
          );
      ...
      removeUserFromGroup: async (groupId, userId) => {
        try {
          const { data: updatedGroup } = await axiosInstance.post(
            `/organizations/user-group/${groupId}/remove_user/`,
            {
              user_id: userId,
            },
          );
